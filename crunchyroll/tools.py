...
def run_n_m3u8dl_re(
    ...
    if audio_only:
        ...
        # Primary stream: select video by resolution, all audio tracks.
        # Use --select-video with a resolution regex to pin the quality.
        # NOTE: --auto-select only picks ONE best audio; we need for=all.
        # Always pick the highest quality video track available.
        cmd += ["--select-video", "for=best"]
        # All audio tracks (multi-dub support)
        cmd += ["--select-audio", "all"]
